...
class Node:
    ...
    def __str__(self) -> str:
        return "{0}: {1} {2} {3} -> heuristic: {4}".format(24 - self.time_left, self.recipe, self.stuff, self.robots, self.heuristic if hasattr(self, 'heuristic') else -1)
...
def solve(file: str) -> int:
    ...
    for (i, recipe) in enumerate(recipes):
        root = Node(None, recipe, (0, 0, 0, 0), (1, 0, 0, 0), 24)
        nodes = [root]
        max_geodes = 0
        ...
        while len(nodes) > 0:
            ...
            #        node = n
            #nodes.remove(node)
            if max_geodes < node.get_geodes() or counter % 10000 == 0:
                print("Run ({5}) - max: {0}, nodes: {1}, time: {2}, robots: {3}, stuff: {4}, heuristic: {6}, geodes: {7}"\
                    .format(max_geodes, len(nodes), node.time_left, node.robots, \
                        node.stuff, counter, node.heuristic, node.get_geodes()))
