...
class Node:
    ...
    def can_move_to(self, other: Node) -> bool:
        ...
        if ord(other.tile) >= ord(self.tile) - 1:
            return True
...
with open("input.txt") as f:
    ...
    start_pos = find_tile(elevation_map, "E")
    start: Node = Node(start_pos, elevation_map, start_pos, None)
    ...
    while len(possibilities) > 0:
        ...
        if current.tile == "a":
            print(start, len(elevation_map[0]), len(elevation_map), current.path_len)
        ...
        for direction in directions:
            ...
            if node == None:
                node = Node(new_pos, elevation_map, start.pos, current)
